...
@Schema(description = "의료비 기록 등록 요청. expenseAmount는 expenseDetails 금액 합계와 같아야 합니다.")
public record ExpenseCreateRequest(

        @Schema(description = "결제 금액(원). 세부 항목 합계와 일치해야 함", example = "77000",
                requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull(message = "결제 금액은 필수입니다.")
        @Min(value = 0, message = "결제 금액은 0원 이상이어야 합니다.")
        Long expenseAmount,

        @Schema(description = "이용 날짜 (yyyy-MM-dd). 미래 날짜 불가", example = "2026-07-06",
                requiredMode = Schema.RequiredMode.REQUIRED)
        @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "yyyy-MM-dd")
        @NotNull(message = "이용 날짜는 필수입니다.")
        LocalDate expenseDate,

        @Schema(description = "결제수단 enum. 화면 표시는 응답 paymentTypeLabel 사용", example = "EASY_PAY",
                allowableValues = {"CARD", "TRANSFER", "VIRTUAL_ACCOUNT", "MOBILE", "EASY_PAY"},
                requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull(message = "결제수단은 필수입니다.")
        PaymentTypeEnum paymentType,

        @Schema(description = "방문 병원/구매처 (최대 255자)", example = "행복동물병원",
                requiredMode = Schema.RequiredMode.REQUIRED)
        @NotBlank(message = "병원(방문처)은 필수입니다.")
        @Size(max = 255, message = "병원명은 255자 이하여야 합니다.")
        String expenseName,

        @Schema(description = "세부 항목 (최소 1개). 금액 합계 = expenseAmount",
                requiredMode = Schema.RequiredMode.REQUIRED)
