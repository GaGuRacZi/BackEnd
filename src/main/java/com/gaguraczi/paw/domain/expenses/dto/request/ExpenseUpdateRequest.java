...
@Schema(description = "의료비 기록 수정 요청. 보낸 필드만 반영. expenseDetails를 보내면 기존 항목 전체 교체")
public record ExpenseUpdateRequest(

        @Schema(description = "결제 금액(원)", example = "77000")
        ...
        @Schema(description = "결제수단 enum", example = "EASY_PAY",
                allowableValues = {"CARD", "TRANSFER", "VIRTUAL_ACCOUNT", "MOBILE", "EASY_PAY"})
        PaymentTypeEnum paymentType,

        @Schema(description = "방문 병원/구매처. 공백만 보내면 실패", example = "행복동물병원")
        @Size(max = 255, message = "병원명은 255자 이하여야 합니다.")
        @Pattern(regexp = "\\S(.*\\S)?", message = "병원명은 공백일 수 없습니다.")
        String expenseName,

        @Schema(description = "세부 항목 전체 교체 목록. 생략 시 기존 유지, 빈 배열은 실패(EXPENSE_400_1)")
        @Valid
